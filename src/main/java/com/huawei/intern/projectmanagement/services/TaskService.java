...
import com.huawei.intern.projectmanagement.services.abstracts.ITaskService;
...
public class TaskService extends GenericService<Task> implements ITaskService {
    ...
    public void deleteTicketOfTask(Long tickeId, Long taskId) {
        ...
        Task task = taskRepository.findById(tickeId).orElse(null);

        if(task != null){
            for(Ticket ticket : task.getTickets() ){
                if( ticket.getName().equals(tickeId)){
    ...
    public void editTask(EditTaskDto dto) throws Exception {
